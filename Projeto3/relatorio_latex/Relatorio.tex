...
o nosso escore foi melhor atingindo o escore com mais de 94\%, a diferença que utilizamos SMOTE, como realizado por Susan Li \cite{TowardsDataScience:2017}. 
...
Chris \cite{Medium:2019}, manter todos os 54 features ou variáveis independentes. Durante o treinamento e no cross-validation (CRV), tiveram ótimo desempenho, com todas as métricas
maiores que 90\%. 
...
Levando-se em conta, também, que o desempenho das métricas no CRV é similar ao de teste, o que garante que a distribuição dos dados seja a mesma.
  ...
  \begin{table}[!ht]       
        ...
        \label{qualy}
            \begin{tabular}{|c|c|c|c|}
                ...
                \hline
                \hline
                \hline 0 age& 40+  & 40 +/- 13 anos & pvalue \textgreater 1\% \\ 
                \hline 1 contact& 10+ & c30257/t2626 &  no(c20020/t12863) \\ 
                \hline 2 duration& 40+ & 9.1 +/- 6.6 min &  no (3.6 +/- 3.4) \\ 
                \hline 3 campaign& 40+ & 1.7 +/- 1.2 &  no(2.6 +/- 2.8)\\ 
                \hline 4 pdays& 40+ & 789 +/- 405 dias &  no(984 +/- 120)\\ 
                \hline 5 previous& 30+ & 0.3 +/- 0.7 &  no(0.1 +/- 0.4)\\ 
                \hline 6 emp.var.rate& 30+ & -1.2 +/- 1.6 &  no(0.2 +/- 0.4)\\ 
                \hline 7 cons.price.idx& 40+ & 93.3 +/- 0.6 &  no(93.6 +/- 0.5)\\ 
                \hline 8 cons.conf.idx& 40+ & -39.8 +/- 5.9 &  no(-40.5 +/- 4.3)\\ 
                \hline 9 euribor3m& 40+& 2.1 +/- 1.7 &  no(3.8 +/- 1.6)\\ 
                \hline 10 nr.employed& 40+ & 5094 +/- 87 & no(5176 +/- 64) \\ 
                ...
                \hline 44 month\_may& 16.89 & 3799 & no(236) \\ 
...
A notação média  (medium) +/- desvio padrão (std)são utilizadas para as features numéricas.

Era de se esperar que se houvesse uma interseção de uma feature com resposta positivo [mínimo, máximo] contra resposta negativa [mínimo, máximo], o pvalue
...
Porém isso não ocorreu para outros casos, o pvalue foi menor que 1\% indicando que haveria que rejeitar a hipótese nula. Mesmo assim, a faixa de valores
foram listadas na Tabela \ref{qualy}
...
\begin{center}
	\begin{table}[!ht]
		\caption{Comparação de Desempenho entre referências sobre o uso de Logistic Regression e suas métricas e estratégias utilizadas.}\label{tab:desempenho2}
		\resizebox{}\columnwidth}{}{%}
		\centering    %% not "\center{...}"
		\begin{tabular}{|c|c|c|c|c|}
			\hline
			Técnicas&Este Projeto&\cite{TowardsDataScience:2017}&\cite{Medium:2019}&\cite{Moro:2014}\\    
			\hline
			\hline
			AUC&0.95&0.74&-&0.71\\     %% no "&" at start of row
			\hline        %% extra \hline at bottom of table
			Accuracy&0.95&0.74&0.91&-\\     %% no "&" at start of row
			\hline        %% extra \hline at bottom of table
			Precision&0.95&0.74&-&-\\     %% no "&" at start of row
			\hline        %% extra \hline at bottom of table
			Recall&0.95&0.74&-&-\\     %% no "&" at start of row
			\hline        %% extra \hline at bottom of table
			F1&0.95&0.74&-&-\\     %% no "&" at start of row
			\hline        %% extra \hline at bottom of table
			SMOTE&yes&yes&no&no\\     %% no "&" at start of row
			\hline        %% extra \hline at bottom of table
			Normalization&yes&no&yes&no\\     %% no "&" at start of row
			\hline        %% extra \hline at bottom of table
			RFE&yes&yes&no&no\\     %% no "&" at start of row
			\hline        %% extra \hline at bottom of table
			A.Q.&yes&no&no&yes\\     %% no "&" at start of row
			\hline
			B.S.&yes&no&no&yes\\     %% no "&" at start of row	
			\hline        %% extra \hline at bottom of table
	\end{tabular}}
\end{table}
\end{center}
...
antes do treinamento da amostra ou dataset com algum modelo, a obtenção das métricas com valores de qualidade depende muito de como é tratado as etapas iniciais.

O ponto forte do nosso projeto foi o procedimento adotado na etapa inicial de preparação dos dados. Os pontos fracos seriam a análise qualitativa, faltaram-nos 
conhecimento de mais ferramentas que pudessem extrair as features mais importantes de forma mais automáticas. A estatística básica do t-test aparentemente é pouco confiável, os valores do
pvalue resultam em valores muito baixo, enquanto se espera valores mais altos, observando-se os valores das médias e desvios padrões das features numéricas, conclui-se que há muita
chance de ter casos de falsos positivos, assim ficava muito difícil extrair as features mais importantes, por isso foi usado o RFE. Tentamos utilizar também o regularization para  
esse propósito, mas sem sucesso.
...
Existe uma boa chance de se obter um bom modelo de predição.
